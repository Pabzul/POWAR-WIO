...
const long sensorUpdateInterval = 5000;  // Intervalo de 5 segundos para actualizar los sensores

unsigned long previousJoystickMillis = 0;
const long joystickInterval = 200;  // Intervalo de 200 milisegundos para revisar el joystick
  ...
  if (currentMillis - previousJoystickMillis >= joystickInterval) {
    ...
    if (isUpPressed || isDownPressed) {
      if (!joystickPressed) {
        ...
        // Cambiar de pantalla
        if (isUpPressed) {
          currentScreen = (currentScreen + 1) % 5; // Asegura que currentScreen esté entre 1 y 4
        } else if (isDownPressed) {
          currentScreen = (currentScreen - 1 + 4) % 4 + 1; // Asegura que currentScreen esté entre 1 y 4
        }
...
void updateSensors() {
  ...
  // Verificar si ha pasado el tiempo suficiente para actualizar los sensores nuevamente
  if (currentMillis - previousSensorUpdateMillis >= sensorUpdateInterval) {
    ...
    // Nota: Aquí asumimos que las funciones Pantalla1(), Pantalla2(), Pantalla3()
    // actualizan las pantallas con los valores de los sensores.
    switch (currentScreen) {
      case 1:
        Pantalla1();
        break;
