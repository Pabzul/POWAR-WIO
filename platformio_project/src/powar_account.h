...
void setupAccount() {
  ...
  while (true) {
    ...
    Serial.printf("[HTTP] POST... code: %d\n", httpCode);
    // successful response
    StreamString res = http.getString();
    ...
    if (type == "success") {
      /**
On success we tell the POWAR device the information it needs for subscribing and
publishing to topics for its modules, this means the module_id and the module
type. We send back (including newlines) ```success:<region_id>:<powar_device_id>
name:<device_name>
...
*/
      ...
      String prop = res.readStringUntil(':');
      Serial.println("prop: " + prop);
      while (prop != "stop") {
        if (prop == "name") {
          String name = res.readStringUntil('\n');
